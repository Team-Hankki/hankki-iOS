...
protocol MenuAPIServiceProtocol {
    ...
    typealias GetMenuResponseDTO = BaseDTO<GetMenuResponseData>
    typealias PostMenuResponseDTO = BaseDTO<PostMenuResponseData>
    
    func getMenu(storeId: Int, completion: @escaping(NetworkResult<GetMenuResponseDTO>) -> Void)
    ...
    func getMenu(storeId: Int, completion: @escaping (NetworkResult<GetMenuResponseDTO>) -> Void) {
        provider.request(.getMenu(storeId: storeId)) { result in
            ...
            case .success(let response):
                let networkResult: NetworkResult<GetMenuResponseDTO> = self.fetchNetworkResult(statusCode: response.statusCode, data: response.data)
